...
def predictor(img_paths, img_dimension=int(os.getenv("img_dimension", "640")), batch_size=1):
    all_preds = []
    for path in img_paths:
        img0 = cv2.imread(path)
        stride = 64
        new_shape = check_img_size(img_dimension, stride)

        img = letterbox(img0, new_shape, stride=stride)[0]
