...
def handle_file_dict_request(unique_id, in_dict):
    '''
        ...
        :return res_path: result file path
    '''
    _write_dir, res_path = get_write_res_paths(unique_id, 0.75 * sum([len(v) for v in in_dict.items()]))
    ...
    for i, (file_name, b64_string) in enumerate(in_dict.items()):
        ...
        file_path = os.path.join(_write_dir, f'{str(i).zfill(len(in_dict) + 1)}.{file_name}')
